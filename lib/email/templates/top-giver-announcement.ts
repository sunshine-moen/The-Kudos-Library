interface TopGiverData {
  recipientFirstName: string;
  kudosCount: number;
  weekLabel: string;
  celebrateUrl: string;
  quoteText: string;
  quoteAuthor: string;
}

export function renderTopGiverAnnouncement(data: TopGiverData): string {
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>You lit up the library this week</title>
</head>
<body style="margin:0;padding:0;background:#F5EEDD;font-family:Georgia,serif;">
  <div style="max-width:600px;margin:0 auto;padding:32px 16px;">

    <div style="text-align:center;margin-bottom:32px;">
      <p style="font-size:11px;letter-spacing:0.1em;text-transform:uppercase;color:#8B7355;margin:0 0 8px;">
        The Kudos Library
      </p>
      <h1 style="font-size:22px;color:#002145;margin:0;">
        You lit up the library this week, ${escapeHtml(data.recipientFirstName)}
      </h1>
    </div>

    <div style="background:#002145;border-radius:4px;padding:28px 32px;margin-bottom:24px;text-align:center;">
      <p style="color:#E69400;font-size:13px;letter-spacing:0.08em;text-transform:uppercase;margin:0 0 12px;">
        ${escapeHtml(data.weekLabel)}
      </p>
      <p style="color:#F5EEDD;font-size:36px;font-weight:700;margin:0 0 4px;">${data.kudosCount}</p>
      <p style="color:#D4C5A9;font-size:14px;margin:0;">
        ${data.kudosCount === 1 ? "kudos given" : "kudos given"}
      </p>
    </div>

    <div style="background:#fff;border-left:4px solid #E69400;padding:20px 24px;border-radius:2px;margin-bottom:24px;">
      <p style="font-size:15px;color:#3D2B1F;margin:0;line-height:1.7;">
        Every book on the shelf started with someone taking a moment to notice.
        Thank you for being that person this week — and for making the library a little richer.
      </p>
    </div>

    <div style="text-align:center;margin-bottom:32px;">
      <a href="${escapeHtml(data.celebrateUrl)}"
         style="display:inline-block;background:#E69400;color:#002145;text-decoration:none;font-size:14px;font-weight:700;padding:12px 28px;border-radius:2px;">
        Keep the energy going
      </a>
    </div>

    <div style="border-top:1px solid #D4C5A9;padding-top:20px;text-align:center;">
      <p style="font-size:13px;font-style:italic;color:#8B7355;margin:0 0 4px;line-height:1.6;">
        &ldquo;${escapeHtml(data.quoteText)}&rdquo;
      </p>
      <p style="font-size:11px;color:#A89565;margin:0;">— ${escapeHtml(data.quoteAuthor)}</p>
    </div>

  </div>
</body>
</html>`;
}

function escapeHtml(str: string): string {
  return str
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;")
    .replace(/"/g, "&quot;");
}
